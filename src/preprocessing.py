...
import numpy as np
import pandas as pd
from src.errors import IllegalArgumentError
...
defaults = {
    ...
    "write_pages": WRITEDataset(
        None,
        transformation_pipeline=TransformationPipeline(
            os.path.join(this_file_dir, os.pardir, "dataset_splits", "write_pages.csv"),
            SIFTPatchExtractor(sigma=3.75),
            "write_pages",
            pipeline_items=[OtsuBinarization()],
            conditional_filter=lambda img: np.sum(img == 255) > int(32 ** 2 * 0.95)
        ), root_dir=os.path.join(os.curdir, "data")),
    "write_script-subset_pages": WRITEDataset(
        None,
        transformation_pipeline=TransformationPipeline(
            os.path.join(this_file_dir, os.pardir, "dataset_splits", "write_script-subset_pages.csv"),
            SIFTPatchExtractor(sigma=3.75),
            "write_script-subset_pages",
            pipeline_items=[OtsuBinarization()],
            conditional_filter=lambda img: np.sum(img == 255) > int(32 ** 2 * 0.95)
        ), root_dir=os.path.join(os.curdir, "data")),
    "write_block-letters-block-capitals-subset_pages": WRITEDataset(
        None,
        transformation_pipeline=TransformationPipeline(
            os.path.join(this_file_dir, os.pardir, "dataset_splits",
                         "write_block-letters-block-capitals-subset_pages.csv"),
            SIFTPatchExtractor(sigma=3.75),
            "write_block-letters-block-capitals-subset_pages",
            pipeline_items=[OtsuBinarization()],
            conditional_filter=lambda img: np.sum(img == 255) > int(32 ** 2 * 0.95)
        ), root_dir=os.path.join(os.curdir, "data"))
}
