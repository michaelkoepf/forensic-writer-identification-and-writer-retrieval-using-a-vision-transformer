...
import patoolib
from src.errors import InvalidHashError, IllegalArgumentError
...
class WriterRecognitionDataset:
    ...
    def extract_archive(self):
        ...
        for filename in self.filenames:
            extract_path = os.path.join(self.root_dir, self.dirname_extracted_files, filename.split(".")[0])

            self._log(f"Check if dataset {self.identifier} has already been extracted...")

            if extract_path in glob.glob(os.path.join(self.root_dir, self.dirname_extracted_files, "*")):
            ...
            self._extract(os.path.join(self.root_dir, filename),
                          os.path.join(self.root_dir, self.dirname_extracted_files))
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.transformation_pipeline:
            ...
            if self.transformation_pipeline(self.root_dir,
                                            [os.path.join(self.dirname_extracted_files, filename.split(".")[0]) for
...
class CVLCroppedDataset(WriterRecognitionDataset):
    ...
    def __init__(self, num_classes_train, **kwargs):
        ...
        super().__init__(**self.meta_data, **kwargs)
